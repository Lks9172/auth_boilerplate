...
export class UserRepository extends Repository<User> {
  ...
  async getUserById(userId: string): Promise<User> {
    ...
    if (!user) throw new NotFoundException(`Can't find User with id ${userId}`);
  ...
  async updatePasswordById(user: User, account: Account): Promise<boolean> {
    ...
    if (!res) throw new Error('password를 변경하는데 실패했습니다.');
  ...
  async updateTokenById(user: User, account: Account): Promise<boolean> {
    ...
    if (!res) throw new Error('token정보를 업데이트하는데 실패했습니다.');
  ...
  async deleteById(user: User): Promise<boolean> {
    ...
    if (!res) throw new Error('user를 삭제하는데 실패했습니다.');
